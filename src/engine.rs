    ...
    T: ActionCallback,
{
    ...
    pub fn play_round(&mut self) {
        ...
        println!(
            "{:?}",
            self.players.iter().map(|p| p.stack).collect::<Vec<Money>>()
        );
    }
    ...
    fn bet(&mut self, player_index: usize, max_bet: Money, min_betsize: Money) {
        match self.callback.callback(Message::RequestAction(player_index)) {
            ...
            Message::Player {
                action: PlayerAction::Raise(new_bet),
                ..
            } => {
                if new_bet - max_bet < min_betsize
                    || self.players[player_index].raise(new_bet).is_err()
                {
                    self.callback
                        .callback(Message::Error(ErrorMessage::BetNotAllowed));
                    self.bet(player_index, max_bet, min_betsize);
                }
            }
            _ => {
                self.callback
                    .callback(Message::Error(ErrorMessage::InvalidResponse));
                self.bet(player_index, max_bet, min_betsize);
            }
        }
    }
